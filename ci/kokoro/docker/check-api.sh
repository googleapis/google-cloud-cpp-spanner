...
if [[ "${CHECK_API:-}" != "yes" ]]; then
  echo
  echo "${COLOR_YELLOW}Skipping API check as it is disabled for this build." \
      "${COLOR_RESET}"
  exit 0
fi
...
check_library() {
  ...
  echo
  echo "${COLOR_YELLOW}Checking API for ${library} library.${COLOR_RESET}"
  ...
  reference_file="${PROJECT_ROOT}/ci/test-api/${old_dump_file}.gz"

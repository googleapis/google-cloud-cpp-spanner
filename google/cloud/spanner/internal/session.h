...
class Session {
 public:
  Session(std::string session_name, std::shared_ptr<Channel> channel,
          ClockNowSource time_now = Clock::now)
 ...
 private:
  ...
  void update_last_use_time() { last_use_time_ = time_now_(); }

  std::string const session_name_;
  std::shared_ptr<Channel> const channel_;
  ClockNowSource time_now_;
  std::atomic<bool> is_bad_;
  Clock::time_point last_use_time_ = time_now_();
};
...
#endif  // GOOGLE_CLOUD_CPP_SPANNER_GOOGLE_CLOUD_SPANNER_INTERNAL_SESSION_H

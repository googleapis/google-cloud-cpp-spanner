...
// A 256-element table mapping all `unsigned char` byte values to a printable
// string. All naturally printable values are used directly. All non-printable
// bytes use their octal escape sequence. Whitespace characters use their short
// form like '\t' and '\n'.
using ByteType = unsigned char;
constexpr auto kByteTableSize = std::numeric_limits<ByteType>::max() + 1;
constexpr std::array<char const*, kByteTableSize> kByteTable = {
...
// We'll index into kByteTable using values of type ByteType, so assert that
// this will never fail.
static_assert(std::numeric_limits<ByteType>::min() == 0, "");
static_assert(std::numeric_limits<ByteType>::max() < kByteTable.size(), "");
  ...
  switch (t.code()) {
    ...
    case google::spanner::v1::BYTES: {
      auto const bytes = internal::BytesFromBase64(v.string_value())
                             ->get<std::vector<ByteType>>();
      os << R"(B")";
      for (ByteType const& byte : bytes) {
        os << kByteTable[byte];
      }
      return os << R"(")";
    }

...
void DmlStandardInsertCommand(std::vector<std::string> const& argv) {
  ...
  spanner::Client client(spanner::MakeConnection(
      spanner::MakeDatabaseName(argv[0], argv[1], argv[2])));
  DmlStandardInsert(std::move(client));
}
...
void RunAll() {
  ...
  google::cloud::spanner::Client client(google::cloud::spanner::MakeConnection(
      google::cloud::spanner::MakeDatabaseName(project_id, instance_id,
                                               database_id)));
